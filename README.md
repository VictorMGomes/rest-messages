# Rest API message standardization package for Laravel applications
...
[<img src="https://github-ads.s3.eu-central-1.amazonaws.com/rest-messages.jpg?t=1" width="419px" />](https://spatie.be/github-ad-click/rest-messages)
...
```bash
php artisan vendor:publish --tag="rest-messages-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="rest-messages-config"
```
...
```bash
php artisan vendor:publish --tag="rest-messages-views"
```

## Usage

```php
$restMessages = new Victormgomes\RestMessages();
echo $restMessages->echoPhrase('Hello, Victormgomes!');
```
